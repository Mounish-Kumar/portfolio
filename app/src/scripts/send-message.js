    ...
    $.ajax({
      ...
      success: function (data, textStatus, jqXHR) {
        if (jqXHR.status === 200 && data && data.status === "SENT") {
          ...
          setTimeout(() => {
            setSuccessMessage(
              "Thanks for your message! I'll get back to you in 48 hours."
            );
            resetFields();
          }, animationDelay);
        } else {
...
var setFieldErrorMessages = function (fieldErrorMessages) {
  if (fieldErrorMessages && fieldErrorMessages.length > 0) {
    fieldErrorMessages.map((fieldErrorMessage) => {
      ...
      $(`[name="${fieldErrorMessage.field}"] > .field-message.error`).html(
        fieldErrorMessage.message
      ); // Field error message
      $(
        `[name="${fieldErrorMessage.field}"] > .field-message.error`
      ).removeClass("hide"); // Show field error message
    });
  }
};
